...
export default class ProjectBusiness extends ParentBusiness {
  ...
  static url: any = {
    ...
    findWithList: '/contract/contractCheck/getKyContractslList',
  ...
  static onQueryValueChange(pthis, context: any): void {
    this.findWithQuery(pthis, {
      multiParam: pthis.data.searchObj.queryValue
    });
  }
  ...
  static findWithQuery(pthis: any, context: RequestParam): void {
    ...
    if (environment.production) {
      ParentBusiness.daoService.getBase(this.url.findWithList, context).subscribe(result => {
        if (result.success == '1') {
          pthis.data.pageList = result.contractList;
          this.setData(pthis);
        }
      });
      ...
      pthis.data.pageList = [{
        ID: '1',
        Projectname: '下行测量导频的发送方式',
        NO: '2018002',
        UndertakeUnitName: '铁科院',
        CommitmentName: '张三',
      }, {
        ID: '2',
        Projectname: '上行测量导频的传输通道科研',
        NO: '2018301',
        UndertakeUnitName: '中铁投',
        CommitmentName: '张三',
      }, {
        ID: '3',
        Projectname: '测量导频的发送方式发明',
        NO: '2019001',
        UndertakeUnitName: '铁科院',
        CommitmentName: '张三',
      }, {
        ID: '4',
        Projectname: '引力波效应实验装置科研',
        NO: '2019001',
        UndertakeUnitName: '信息所',
        CommitmentName: '张三',
      },];
    }
  }
  ...
  static navigatorDetail(pthis: any, context: any): void {
    console.log(context);
    ParentBusiness.router.navigate(['/projectdetails'], {
      queryParams: {
        ID: context.ID
      }
    });
